...
These sources were consumed by the batch scaffold prompt (see `README.md`) to generate the populated `docs/` tree. They are kept here for traceability — if a `docs/` document seems to contradict a source, the `docs/` document is authoritative (it reflects current decisions, not initial inputs).
