...
export default function ProductHiddenInput(props: StringFieldProps) {
  ...
  let message
  if (!isActive) {
    message = <>It does not have an <code>active</code> status in Shopify.</>
  }
  if (isDeleted) {
    ...
    <Card padding={4} radius={2} shadow={1} tone="critical">
      <Flex align="flex-start">
        ...
        <Box flex={1} marginLeft={3}>
          <Box><Text size={2} weight="semibold">This product is hidden</Text></Box>
          <Stack marginTop={4} space={2}><Text size={1}>{message}</Text></Stack>
          {!isDeleted && shopifyProductUrl && (
            <Box marginTop={4}>
              <Text size={1}>{'\u2192'} <a href={shopifyProductUrl} target="_blank" rel="noreferrer">View this product on Shopify</a></Text>
            </Box>
          )}
        </Box>
      </Flex>
